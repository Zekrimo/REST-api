...
double runningTotalSensor1 = 0;
double runningTotalSensor2 = 0;
double runningSquaredTotalSensor1 = 0;
double runningSquaredTotalSensor2 = 0;
...
void addToBuffer(CircularBuffer *cb, int item, int sensor)
{
  ...
  if (sensor == 1)
  {
    ...
    printf(" | runningCountSensor1: %d", runningCountSensor1);
  ...
  else if (sensor == 2)
  {
    ...
    printf(" | runningCountSensor2: %d", runningCountSensor2);
...
// function should return the average of all posts on the specified sensor. not soley the buffer
...
double getRunningAverage(int sensor)
{
  if (sensor == 1)
  {
    ...
    return runningCountSensor1 > 0 ? runningTotalSensor1 / runningCountSensor1 : -1.0;
  }
  else if (sensor == 2)
  {
    printf("runningTotalSensor2: %f", runningTotalSensor2);
    ...
    return runningCountSensor2 > 0 ? runningTotalSensor2 / runningCountSensor2 : -1.0;
  }
  return -1.0;
}
...
// function should return the standard deviation of all posts on the specified sensor. not soley the buffer
...
//   return stdev; // Return the calculated standard deviation
//                 // without rounding
// }
...
void setBufferSize(CircularBuffer *buffer, int newSize, int sensor)
{
  ...
  int count = buffer->count < newSize ? buffer->count : newSize;

  for (int i = 0; i < count; i++)
  {
    newData[i] = buffer->data[(buffer->head + i) % buffer->bufferSize];
  }
...
struct response handleRequest(struct stream stream)
{
  ...
  if (strcmp(method, "PUT") == 0)
  {
    ...
    else if (strcmp(uri, "/config/cbuffsize") == 0)
    {
      ...
      setBufferSize(&sensorBuffer1, newSize, 1); // example size
      setBufferSize(&sensorBuffer2, newSize, 2); // example size
  ...
  else if (strcmp(method, "POST") == 0)
  {
    ...
    if (strncmp(uri, "/sensors/1", 10) == 0)
    {
      addToBuffer(&sensorBuffer1, value, 1); // Use value from body
      printf("Posted measurement %d to sensor 1\n", value);
      return (struct response){.code = CREATED_201_POST_MEASUREMENT};
    }
    else if (strncmp(uri, "/sensors/2", 10) == 0)
    {
      addToBuffer(&sensorBuffer2, value, 2); // Use value from body
