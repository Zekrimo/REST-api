...
void resetRunningStatistics(int sensor) {
  logmsg("reset running statistacs");
  if (sensor == 1) {
...
double getRunningStandardDeviation(int sensor) {
  if (sensor == 1) {
    ...
    double variance =
      (runningSquaredTotalSensor1 / runningCountSensor1) - (mean * mean);

    logmsg("running total");
    logDouble(runningTotalSensor1);
    logmsg("count");
    logDouble(runningCountSensor1);
    logmsg("mean");
    logDouble(mean);
    logmsg("variance");
    logDouble(variance);
    ...
    if (sqrt(variance) < 0) {
      // Log error or adjust due to precision issues
      logmsg("variance is smller then 0");
    }
    ...
    logmsg("running total");
    logDouble(runningTotalSensor2);
    logmsg("count");
    logDouble(runningCountSensor2);
    logmsg("mean");
    logDouble(mean);
    logmsg("variance");
    logDouble(variance);
...
// Function to log integer
void logInt(int num) {
  char buffer[50];  // Adjust the size according to the expected number range
  sprintf(buffer, "Integer: %d", num);
  logmsg(buffer);
}

// Function to log double
void logDouble(double num) {
  char buffer[50];                       // Adjust the size for expected double precision
  sprintf(buffer, "Double: %.3f", num);  // Using %.3f for three decimal places
  logmsg(buffer);
}