...
{
  char line[256]; // Buffer to store the line read from the
                  // streams
  char method[10];
  char uri[50];
  char body[256];
  ...
  // Try to read the method, URI, and protocol from the
  // stream This is a simplified approach and assumes
  // well-formed HTTP requests
  if (3 !=
      sscanf(line, "%9s %49s %9s", method, uri, protocol))
  {
  ...
  if (strcmp(method, "PUT") == 0 &&
      strcmp(uri, "/config/mode") == 0)
  {
    if (strcmp(uri, "/config/mode") == 0)
    {
      // Code to switch mode based on body content; assuming
      // body parsing is handled
      //currentMode = MODE_PASSIVE; // or MODE_ACTIVE depending on the
                        // body content
      return (struct response){
          .code = 201}; // Adjust response code as needed
    }
    else if (strcmp(uri, "/config/cbuffsize") == 0)
    {
      // Code to adjust buffer size based on body content
      //setBufferSize(&sensorBuffer1, 10); // or any other value based on the
                         // body content
      //setBufferSize(&sensorBuffer2, 10); // or any other value based on the
                         // body content
        ...
        strncmp(uri, "/sensors/", 9) == 0)
    {
      ...
      int sensorId = 0;
      char action[20]; // For actions like "avg", "stdev",
                       // "actual"

      // Parse the URI to extract sensor ID and the action
      // (e.g., /sensors/1/avg)
      ...
      // Check if the target buffer was set and perform the
      // action
      if (targetBuffer != NULL)
      {
        if (strstr(uri, "/avg"))
        {
          return (struct response){
              .code = OK_200_GET_AVG,
              .get_avg = getBufferAverage(targetBuffer)};
        }
        if (strstr(uri, "/stdev"))
        {
          return (struct response){
              ...
              .get_stdev =
                  getBufferStandardDeviation(targetBuffer)};
        }
        if (strstr(uri, "/actual"))
        {
          return (struct response){
              .code = OK_200_GET_ACTUAL,
              .get_actual = getBufferActual(targetBuffer)};
        }
      }
    }
  }
...
void addToBuffer(CircularBuffer *cb, int item)
{
  ...
  {
    // Handle buffer full (e.g., overwrite oldest data or
    // expand policy)
  }
}
...
double getBufferAverage(CircularBuffer *buffer)
{
  if (buffer->count == 0)
  {
    return 0.0; // Avoid division by zero; adjust based on
                // your error handling
  }
...
int getBufferActual(CircularBuffer *buffer)
{
  if (buffer->count == 0)
  {
    return -1; // Indicates buffer is empty; adjust based on
               // your error handling
  }

  // The tail points to the next empty slot, so subtract 1
  // to get the index of the last added item
  int actualIndex =
      (buffer->tail - 1 + buffer->bufferSize) %
      buffer->bufferSize;
